...
script_start = time.perf_counter() #see how long it takes to run
...
def dna_win_rate(dna: dict ,trials: int) -> int:
    total_player_wins = 0
    deck = build_deck() #new deck and is replaced when it is less than 52
...
def main():
    history = [] #stores the best_win_rates for graphing and analysis
    population = 100
    trials = 50000
    gen_count = 20
    generation = 0
    average_win_rate = [] #store the average win rate of each generation
    ...
    while generation != gen_count:
        ...
        for i in range(population):
            ...
            average += (win_rates[i][0] / trials) #gets the average win rate of each generation
        average /= population #actual average has been calculated each generation
        average_win_rate.append(average*100) #convert average to percentage
        win_rates.sort()
        win_rates.reverse() #make best strategies first
        print(f"Best win_rate of {(generation)}: {win_rates[0][0]/trials:.3f}")
        ...
        #getting the average
        history.append(best_win_rate)
        selection.clear()
        selection = win_rates[:cutoff] #only selects to the top, depends on variable cutof
        new_dna.clear() #clear dna for new generation
        ...
        generation += 1
    print(f"The best win rate is {best_win_rate:.6f}%")
    plot_results(history, gen_count, average_win_rate)
    print(best_dna)
    script_end = time.perf_counter()
    total_time = script_end - script_start
    print(f"The average across the generation list {average_win_rate}")
    print(f"\n\n[Total Run Time was:{total_time:.3f} seconds]")
...
def mutation(dna: dict,mutation_rate: int):
    for cards,action in dna.items():
        if random.randint(1,100) <= mutation_rate: #efectively mutation rate is a percentage, mutation_rate = 1 means 1% 0.1 means 0.1% 5 means 5% mutation rate
            if dna[cards] == "H":
                dna[cards] = "S"
            else:
                dna[cards] = "H"
def plot_results(history: list, gen_count: int, average_win_rate: list):
    generation_list = [i for i in range(1,gen_count+1)]
    #generation is x axis and y axis is the best_win_rate
    plt.plot(generation_list,history, label="best_win_rate")
    plt.plot(generation_list,average_win_rate, label="average_win_rate")
    plt.xlabel("Generation")
    plt.ylabel("Probability %")
    plt.title("Probability over the generations")
    plt.legend() #distinguish average win rate and best win rate
    plt.show()
def sleep(): #sleep function for overnight running when finished
    ctypes.windll.powrprof.SetSuspendState(0, 1, 0)
if __name__ == "__main__":
    main()
#the limit is reached, of 43.7 that is the highest reached over a 100 generations with pop 100 elitisim at 20% and trials/rounds per strategy(dna) is 3000.
